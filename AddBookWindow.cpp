...
LRESULT CALLBACK AddBookWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CREATE:
    {
        for (int i = 0; i < 6; ++i)
        {
            CreateWindowW(L"STATIC", fieldLabels[i].c_str(), WS_VISIBLE | WS_CHILD, 20, 20 + i * 50, 180, 20, hwnd, nullptr, nullptr, nullptr);

            const wchar_t *defaultText = (i == 2 ? L"978- - - - " : L"");

            int width = (i == 0) ? 295 : 250;
            hEditFields[i] = CreateWindowW(L"EDIT", defaultText, WS_VISIBLE | WS_CHILD | WS_BORDER, 200, 20 + i * 50, width, 20, hwnd, nullptr, nullptr, nullptr);
        }
        ...
        CreateWindowW(L"BUTTON", L"Agregar Libro", WS_VISIBLE | WS_CHILD, START_X + BUTTON_WIDTH + 20, BUTTON_Y, BUTTON_WIDTH, BUTTON_HEIGHT, hwnd, (HMENU)1, nullptr, nullptr);
    ...
    case WM_COMMAND:
        if (LOWORD(wParam) == 1)
        {
            ...
            string isbnStr = WStringToString(values[2]);
            const char *paramValues[1] = {isbnStr.c_str()};

            PGresult *res = PQexecParams(conn, "SELECT COUNT(*) FROM libros WHERE isbn = $1", 1, nullptr, paramValues, nullptr, nullptr, 0);
            ...
            string titulo = WStringToString(values[0]);
            string autor = WStringToString(values[1]);
            string isbn = WStringToString(values[2]);
            string editorial = WStringToString(values[3]);
            string año = WStringToString(values[4]);
            string materia = WStringToString(values[5]);
            ...
            res = PQexecParams(conn, "INSERT INTO libros (titulo, autor, isbn, editorial, año, materia) VALUES ($1, $2, $3, $4, $5, $6)", 6, nullptr, insertParams, nullptr, nullptr, 0);
            ...
            {
                wstring msg = L"El libro " + values[0] + L" ha sido agregado exitosamente";
...
void ShowAddBookWindow(HINSTANCE hInstance, const wstring &username)
{
    ...
    wstring title = L"Agregar Libro - Usuario: " + username;
